  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-[#D4AF37]/10 px-4 sm:px-12 py-3 sm:py-4 transition-all duration-500">
      <div className="max-w-7xl mx-auto">
        {/* Mobile: Grid (2 rows), Desktop: Flex (we handle via hidden/block or just keep flex/grid logic) */}
          ...
          >
            <div className={`relative transition-all duration-500 ${isSearching ? 'scale-[1.01]' : ''}`}>
              <input 
                ...
                className="w-full bg-[#FDFCFA] border border-[#D4AF37]/10 px-4 sm:px-8 py-2 sm:py-3 rounded-none text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-[#D4AF37]/30 transition-all duration-500 tracking-wide italic text-xs sm:text-sm" 
              />
