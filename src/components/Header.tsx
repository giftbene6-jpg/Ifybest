  ...
  return (
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-[#D4AF37]/10 px-4 sm:px-12 py-3 sm:py-4 transition-all duration-500">
      <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-y-4">
        ...
        <Link 
          href="/"
          className="text-2xl sm:text-4xl font-serif tracking-widest lux-gradient-text hover:scale-105 transition-all duration-500 font-medium order-1"
          style={{ fontFamily: 'var(--font-playfair)' }}
        >
          IFYBEST
        </Link>

        {/* Navigation & User Area - Keep next to logo on mobile */}
        <div className="flex items-center space-x-4 sm:space-x-10 order-2 sm:order-3">
          <Link
            href="/basket"
          ...
          <ClerkLoaded>
            ...
            {user ? (
              <div className="flex items-center border-l border-[#D4AF37]/10 pl-4 sm:pl-10 h-6 sm:h-8">
                <UserButton 
                  appearance={{ 
                    elements: { 
              ...
              <div className="border-l border-[#D4AF37]/10 pl-4 sm:pl-10">
        ...
        <form 
          action="/search" 
          method="GET" 
          className="w-full sm:flex-1 sm:mx-16 order-3 sm:order-2"
        ...
        >
          <div className={`relative transition-all duration-500 ${isSearching ? 'scale-[1.01]' : ''}`}>
            <input 
              ...
              className="w-full bg-[#FDFCFA] border border-[#D4AF37]/5 px-6 sm:px-8 py-2.5 sm:py-3 rounded-none text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-[#D4AF37]/30 transition-all duration-500 tracking-wide italic text-xs sm:text-sm" 
            />
